...
function enterWords(){ //検索条件入力に関するfunction
  ...
  //最初のプロンプトを表示
  var response = ui.prompt("検索条件の個数を入力してください。\n※全件表示する場合は１から５の整数を押して次へ進んでください。",btn1);
...
function inputOnPrompt(text1){
  ...
  //最初のプロンプトを表示する。
  var response1 = ui.prompt("検索条件を1つ入力してください。\n※全件表示する場合は、空白のままOKボタンを押してください。",btn1);
  ...
  if(response1.getResponseText() == ""){
    var response2 = Browser.msgBox("全件表示でよろしいですか？",btn1);
    ...
    Browser.msgBox("検索条件に半角全角スペースを入力することはできません。\\n検索条件の個数選択からやり直してください。");
    return;
  }
  ...
  for(i = 1; i < text1; i++){
    ...
    if(response3.getResponseText() == "" || response3.getResponseText().toString().indexOf(" ") >= 0 || response3.getResponseText().toString().indexOf("　") >= 0){
      Browser.msgBox("2つ目以降の条件入力欄で、空白・半角全角スペースを入力することはできません。\\n検索条件の個数選択からやり直してください。");
      return;
    }
  ...
  //入力値の最終チェックを行う。
  let btnCheckFor_searchValue = Browser.msgBox(inputValues+"検索値は以上でよろしいですか？\\n※数値が入力されている場合、数値は社員番号と見なされるため、\\nその他の検索条件は無視されます。",btn1);
...
function displayRecord(outputList){
  ...
  //前回表示エリアのチェックボックスを消去する。
  sheet3.getRange(3,1,sheet3.getMaxRows(),1).removeCheckboxes();

  //前回の表示エリアを取得
  const sheet3DisplayBeforeArea = sheet3.getRange(3,2,sheet3.getRange(3,2).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRow(),5);

  //前回の表示エリアを空にする。
  sheet3DisplayBeforeArea.setValue("");

  if(outputList[0].length == 0){
    ...
    var sheet3DisplayArea = sheet3.getRange(3,2,outputList.length,outputList[0].length);

    //チェックボックス表示範囲を計算して取得し、そこにチェックボックスを表示する。
    sheet3.getRange("A3:A"+(2+outputList.length)).insertCheckboxes();
...
function decidingSearchArea(sheet){
  ...
  if(sheetName == "リスト"){
    ...
    if(sheet2FirstRecordPK.isBlank() == true){
      ...
      Browser.msgBox("リストシート内にレコードが存在しません。");
    ...
    //sheet3の最初のレコードの値を取得。これの有無でシート内にレコードがあるかを判定する。
    const sheet3FirstRecordPK = sheet.getRange(3,2);
    const sheet3FirstRecordSurname = sheet.getRange(3,3);
    const sheet3FirstRecordName = sheet.getRange(3,4);
    const sheet3FirstRecordSyokusyu = sheet.getRange(3,5);
    const sheet3FirstRecordWhere = sheet.getRange(3,6);

    //シート内にレコードがない場合、処理を終了する。
    if(sheet3FirstRecordPK.isBlank() && sheet3FirstRecordSurname.isBlank() && sheet3FirstRecordName.isBlank() && sheet3FirstRecordSyokusyu.isBlank() && sheet3FirstRecordWhere.isBlank()){

      Browser.msgBox("参照シート内にレコードが存在しません。");
    ...
    //データが存在している範囲の最終行の行番号を取得。
    //-2することでレコードの行数を獲得
    var sheetLastRow = sheet.getLastRow()-2;

    //データが存在している範囲の最終列の列番号を取得
    var sheetLastColumn = 6;
